...
let change_state_substitution substitution state = 
  State.make {goals = state.goals; productions = state.productions;
   substitutions = substitution; orig_vars = state.orig_vars}
let change_state_goals goals state = State.make 
{goals = goals; productions = state.productions; substitutions = state.substitutions;
...
let change_state_production production state = 
  State.make {goals = state.goals;productions = production ;
  substitutions = state.substitutions; orig_vars = state.orig_vars}
    ...
    | Some substitution ->(
      ...
      | Some env -> (
        if (List.length body = 1) then (
          ...
          | (True :: ys) -> SMMonad.map2 (List.append) (eval_query (SMMonad.modify_state 
          (change_state_goals_substitution (sub_lift_goals substitution goal_list) env))) rest
          (*if rule wasn't a fact then we have more subgoals from the body of the rule to prove*)
          | _ -> SMMonad.map2 (List.append) (eval_query (SMMonad.modify_state 
          (change_state_goals_substitution ((sub_lift_goals substitution body) @
           (sub_lift_goals substitution goal_list)) env))) rest
        )
          (*if rule wasn't a fact then we have more subgoals from the body of the rule to prove*)
          else SMMonad.map2 (List.append) (eval_query (SMMonad.modify_state 
          (change_state_goals_substitution ((sub_lift_goals substitution body) @ 
          (sub_lift_goals substitution goal_list)) env))) rest
      )
      | _ -> rest
